...
'''Initial conditions'''
z[0, 0] = 0  # Initial car position 
z[0, 1] = 0  # Initial pendulum position(rad)
z[0, 2] = 0   # Initial speed velocity 
z[0, 3] = 0  # Initial pendulum speed (rad/s)
ie_th = 0
ie_x=0
...
for i in range(n - 1):
    ...
    x_dot = z[i, 3]  #Posición del péndulo
    th    = z[i, 1] #Velocidad del carro
    th_dot= z[i, 2] #Velocidad del péndulo
    ...
    zdot[2] = (u[i,0]+m*lc*th_dot**2*(np.sin(th)) -m*lc*th_dot*np.cos(th)-b1*x_dot) / M+m
...
plt.subplot(325)
plt.plot(t, u[:, 0], 'b', lw=2)
plt.legend([r'$u_{car}$'], loc=1)
plt.ylabel('Control signal car')
plt.xlabel('Time')

plt.subplot(326)
plt.plot(t, u[:, 1], 'r', lw=2)
plt.legend([r'$u_{pendulum}$'], loc=1)
plt.ylabel('Control signal pendulum')
plt.xlabel('Time')
plt.show()
